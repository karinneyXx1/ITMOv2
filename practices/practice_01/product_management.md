...
- правила репозитория, не относящиеся к этому PR: соглашения именования БД, i18n frontend-строк и другие правила репозитория вне Context Pack для этой задачи.
...
Feature: AI-ревью diff через POST /api/reviews (ReviewService)
  ...
  Scenario: Позитивный — секреты не уходят во внешний LLM
    ...
    And промпт содержит маркер [REDACTED] (SEC-1, хороший пример из `context.md`)
...
- Что проверил студент и какие исправления поручил агенту: use cases, sequence-диаграмму и gherkin-сценарии агент сформировал по master prompt P1-03. Сама проверила sequence-диаграмму на соответствие реальному потоку из diff (Dev → API → ReviewService → LLM → Log) и на синтаксические ошибки — рендерится корректно. Сверила упомянутые строки review_service.py:14 с кодом — совпадают, исправлений агенту не потребовалось.